...
- Light normalization is allowed: you may map obvious answer labels such as "final answer", "prediction", "result", "winner", or similar wording into the canonical `text` field.
- If the raw model output contains reasoning plus a final answer, extract the final answer.
- If multiple candidate predictions appear, prefer the last one presented as the final answer.
- Do not invent or complete missing prediction content.
- For this task, extract the final predicted outcome text into `text`.
